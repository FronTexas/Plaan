...
public abstract class TouchExplorationHelper<T> extends AccessibilityNodeProviderCompat
    ...
    /**
     * Computes whether the specified {@link Rect} intersects with the visible portion of its parent
     * {@link View}. Modifies {@code localRect} to contain only the visible portion.
     *
     ...
     * Bundle)}. <p> The helper class automatically handles focus management resulting from {@link
     ...
     * event text by doing one of the following: <ul> <li>appending text to {@link AccessibilityEvent#getText()}</li>
     * <li>populating a description with {@link AccessibilityEvent#setContentDescription(CharSequence)}</li>
     * </ul> </p>
     *
     ...
     * using {@link AccessibilityNodeInfoCompat#setBoundsInParent(Rect)} </ul>
     *
